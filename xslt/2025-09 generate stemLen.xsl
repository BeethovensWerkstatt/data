...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- this file needs to run on every source document.  -->
    ...
    <xsl:variable name="siglum" select="$base.dir[last() - 1]" as="xs:string"/>
    <xsl:variable name="nk" select="doc($current.dir || '/../Notirungsbuch_K/Notirungsbuch_K.xml')" as="node()"/>
    
    <xsl:variable name="surfaces" select=".//mei:surface" as="element(mei:surface)+"/>
    <xsl:variable name="layouts" select=".//mei:layout" as="element(mei:layout)+"/>
    
    <xsl:variable name="pages" as="node()*">
        <xsl:for-each select="$surfaces">
            <xsl:variable name="surface" select="." as="element(mei:surface)"/>
            <xsl:variable name="ref" select="'./../' || $siglum || '/' || $siglum || '.xml#' || $surface/@xml:id" as="xs:string"/>
            <xsl:variable name="nk.folium" select="$nk//mei:foliaDesc//mei:*[$ref = @*]" as="node()?"/>
    ...
    <xsl:template match="/">
        <xsl:for-each select="$pages">
            ...
            <xsl:variable name="folder" select="$current.dir || '/annotatedTranscripts/'" as="xs:string"/>
            <xsl:variable name="fileNamePattern" select="'?select=' || $siglum || '_p' || $padded.pageN || '_wz*_at.xml'" as="xs:string"/>
            ...
            <xsl:message select="'Page ' || $padded.pageN || ': ' || count($docs) || ' / ' || count($docPaths) || ', svg: ' || exists($svg)"/>
                    ...
                    <xsl:for-each select="$linked.surfaces">
                        ...
                        <xsl:choose>
                            <xsl:when test="exists($page)">
                                <xsl:variable name="svg" select="doc($current.dir || substring($page/@svg, 2))" as="node()"/>
    ...
    <xsl:template match="mei:beamSpan" mode="dt1">
        ...
        <xsl:variable name="rastrumId" select="$def/@decls => substring-after('#')" as="xs:string"/>
        <xsl:variable name="staff" select="$page//staff[@xml:id = $rastrumId]" as="node()?"/>
        <xsl:if test="not($staff)">
